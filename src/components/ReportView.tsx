...
export function ReportView({ report, sourceLabel, onExport, onPrint, onBack }: { report: InsightReport; sourceLabel: string; onExport: () => void; onPrint: () => void; onBack: () => void }) {
  const state = report.evidenceCoverage.totalClaims === 0 || report.opportunityScore < 40 ? '暂缓进入' : report.opportunityScore < 60 ? '补充证据' : '建议进入验证'
  return <section className="workspace-page report-page">
    ...
    <article className="print-report">
      ...
      <div className="report-decision"><div><span>市场进入建议</span><h2>{state}</h2><p>{report.recommendation}</p></div><strong>{report.opportunityScore}<small>/100</small></strong></div>
